...
async function getCode(html, name) {
  ...
  for (let script of scripts) {
    console.log(script.rawAttributes.src);
...
function runFunction(fun) {
  function importer(mod) {
    ...
    } else if (mod === 944114 || mod === 441153) {
      // empty object
      return getProxy(
        {
          A: getProxy(
            {
              "getters.auth/authUserId": 423126404,
            },
            "Z"
          ),
        },
...
async function testAPI(path, rules) {
  rules = {
    ...
    ...{
      "user-id": "423126404",
      "x-bc": "204bfb51fb9a6acda6570488cd3dc5a7566001e9",
      cookie:
        "_cfuvid=Q7kU7tEcaNPQdN4Ef3j2j8xUCwTbnwNhctNcSxNloNE-1718052012433-0.0.1.1-604800000; fp=204bfb51fb9a6acda6570488cd3dc5a7566001e9; lang=en; csrf=ARcpSVj312c722360066f7872da7f2e11bdb5703; sess=tcd3m8k14dcc9gip5o74erfjen; auth_id=423126404; st=7030fa2c49d883700d0cede67f492bbd5dc11e0b29e9fada4c344deaecfba7b7; __cf_bm=ZPP6xj.ppLpp4B4Zwxv.VSlQllgtirszR6E3VqZg.Y8-1718145380-1.0.1.1-GTgHj2DiGMtb1jP78w32T.u738rbQIIGOaEXvMvsr.x8XER19HiY8MwgYBEBCVyKRjhq3JxsVV1CyARB_Rjyfg",
      "user-agent":
